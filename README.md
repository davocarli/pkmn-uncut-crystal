...
| 5 | ✅ **Safari Zone** (`src/uc/features/safari.asm`, user-written in dictation; `cut_encounters.asm` table, user's species). **No header hijack needed:** the gate (`SAFARI_ZONE_FUCHSIA_GATE_BETA`) and the zone (`SAFARI_ZONE_BETA`) are complete maps in the ROM, wired to Fuchsia City by warps; only the gate's door block in Fuchsia is a wall variant and the zone has no wild table. Frame hook: while in Fuchsia write the open-door block (`$3A`) into `wOverworldMapBlocks` at the gate's door (`$C874`, WRAM0), and while in the zone write the PARK doorway block (`$0A`, warp carpet) under the zone's two exit warps (`$C8E7–8`), whose tiles are plain floor in the ROM; re-applied every frame since every map setup re-copies the blocks. Step hook: on the zone's tall/long grass roll 25/256, then queue `loadwildmon placeholder, 22+rand(0..7); startbattle; reloadmapafterbattle; end`, which the encounter override swaps from the `cut` table (8 species = 256). The game's own roll on the same step zeroes `wTempWildMonSpecies`, which is why the script sets the species itself. **§7.6 framework deferred** to the first blocks-only map (item 7/8). | 18 | 200 + 120 + 47 | **150 measured** (113 + 28 + 6 walker + 3 tables) | 59% under; harness `safari.py` 17 checks 2026-09-22; in-game confirmation pending. Bottom-row damage of the zone map untouched |
