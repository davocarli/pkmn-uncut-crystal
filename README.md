...
| 1 | ✅ **Encounter override framework** + exclusives (`src/uc/features/encounters.asm`; user wrote the match loop, claude the gates and method classifier). Entry = map, **method bitmask**, species, chance/256 (5 B); chances on the same map form adjacent bands. Methods: grass, cave, surf, old/good/super rod, headbutt, from `wBattleType` + `wFishingRodUsed` + `wPlayerState` + `wEnvironment`. Five entries: Mankey and Mareep on Route 42 grass (2.0% each), Vulpix on Route 37 grass (2.3%), Girafarig on Route 43 grass (2.3%), Remoraid on Route 44 super rod (1.6%) | 2 | est. 90 + 25 | **228 measured** (202 + 26) | **checkpoint: 98% over estimate.** 11 method scenarios + band distribution verified in the headless harness 2026-09-21; in-game confirmation pending |
| 2 | ✅ **Zones framework** + Viridian Forest (`src/uc/features/zones.asm`, user-written with claude dictating; `kanto_zones.asm`, `kanto_encounters.asm` data). A zone is a rectangle on a map with an id and an optional environment to force; the forest zone forces CAVE so the game rolls encounters on every tile of the tree maze using Route 2's own table, and the encounter override treats the zone as a map of its own (`ZONE_GROUP, id`) whose 7 entries sum to 256, i.e. a full custom table at 5 B/slot. Environment is re-forced every frame and restored on exit only if still on the same map. Story phone calls are deferred while inside, as in a real cave. | 3 | est. 90 + 20 + 17–47 | **245 measured** (zones 200 + 9 + 36) | Over, but the zones framework is reusable by every later map. Forest species are a placeholder (R/B forest). Verified in the harness 2026-09-21 (10 scenarios incl. module flags); in-game confirmation pending |
