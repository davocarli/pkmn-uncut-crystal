...
| `00:AE6B–B1FF` | 917 | slot 4 image (UC Runtime), loaded to `04:D200` by `core`'s Init; runtime `$D200` (168 B), GS Ball `$D300` (45 B), Trainer House `$D340` (212 B), Encounters `$D420` (247 B), Version Exclusives table `$D520` (26 B) as of 2026-09-21; 826 of 917 used (`$D53A–$D594` free, 90 B). Runtime is now 225 B (module byte `UCModules` at `$D209`, walker gate). Options menu (268 B at `$D480` + 420 B blob in window 2) shelved 2026-09-20: `src/uc/shelved/ucoptions.asm` |
