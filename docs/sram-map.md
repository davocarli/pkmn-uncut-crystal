...
| `00:AE6B–B1FF` | 917 | slot 4 image (UC Runtime), loaded to `04:D200` by `core`'s Init; runtime `$D200` (202 B), GS Ball `$D300` (38 B), Trainer House `$D340` (208 B, frame half then `UCTrainerHouseStep`), Encounters `$D420` (245 B), window loader `UCWindows` `$D520` (108 B, 9 rows) as of 2026-09-22; 801 of 917 used, free in crumbs only (`$D2CA` 54, `$D326` 26, `$D410` 16, `$D515` 11, `$D58C` 9). **2026-09-22:** feature images are `"UC"` + code (no `jp` header) and encounter tables carry the method once per map block; 99 B returned across the 15 images. The Version Exclusives table moved to window 6 (`$D9D0`) to make room for the loader table. Options menu (268 B at `$D480` + 420 B blob in window 2) shelved 2026-09-20: `src/uc/shelved/ucoptions.asm` |
| `02:BE30–BFFF`, `03:BEEC–BFFF`, `00:BF12–BFFF` | 464 + 276 + 238 | windows 3, 4, 5 → `04:D800`, `04:DA00`, `04:D108`, loaded by `UCWindows` (2026-09-21). Window 3: roaming framework `UCRoam` `$D800` (220 B, of which the two 21 B region sets live in SRAM at `02:BEDC–BF05` and are written in place; the bank-4 copy is stale), Kanto roamers `UCKantoRoamers` `$D8E4` (175 B), cut encounter table `UCCutEncounters` `$D9A0` (21 B); 416 of 464 used, 27 B free at `$D9B5` plus crumbs `$D8DC` 8 and `$D993` 13. Window 4: starter roamer `UCStarterRoamer` `$DA00` (100 B), Safari Zone `UCSafari` `$DA70` (61 B, step only since 2026-09-22: its door and entrance blocks live in the block override), 103 B free at `$DAAD` plus 12 at `$DA64`. Sizes as of 2026-09-22. Window 5 empty (unusable until the bin-offset rule, see above). Skipped units leave holes by design. |
| `00:AC30–AC5F` | 48 | window 6 → `04:D9D0` (2026-09-21): Version Exclusives table `UCExclusivesTable` `$D9D0` (27 B), 21 B free |
| `01:AB83–AD0C`, `00:BD83–BF0C`, `01:B160–B25F`, `00:ADA4–AE6A` | 394 + 394 + 256 + 199 | windows 7, 8, 9, 10 → `04:DB14`, `04:DCA2`, `04:DE30`, `04:DF30` (2026-09-21). Window 7: block override `UCBlocks` `$DB14` (240 B, with the fade-time redraw), cut block table `UCCutBlocks` `$DC10` (58 B: Fuchsia door + the Safari entrance, 17 cells), both 2026-09-22; 88 B free at `$DC4A` plus 12 at `$DC04`. Windows 8, 9, 10 empty. Window 10 is the slack after the 313 B core image; the patcher refuses a core image that reaches it. **Every registered window costs 7 B in the loader table and 7 B of mail code: remove the ones still empty when the feature work is done and the documentation is being finished.** |
