...
| `00:AE6B–B1FF` | 917 | slot 4 image (UC Runtime), loaded to `04:D200` by `core`'s Init; runtime `$D200` (168 B), GS Ball `$D300` (45 B), Trainer House `$D340` (212 B), Encounters `$D420` (253 B, 3 B slack before the next image), Version Exclusives table `$D520` (28 B) as of 2026-09-21 (grouped format); 828 of 917 used (`$D53C–$D594` free, 88 B). Runtime is now 228 B (module byte `UCModules` at `$D209`, walker gate, radio entry). Options menu (268 B at `$D480` + 420 B blob in window 2) shelved 2026-09-20: `src/uc/shelved/ucoptions.asm` |
