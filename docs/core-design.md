...
Slot 4 header at `04:D200`: `db "U4"` signature (`UC_SLOT4_SIG_1/2`), `jp Frame4` at `$D202`, `jp Step4` at `$D205`. `UCInit` always copies the whole slot-4 window from SRAM (917 B ≈ 12k M-cycles, once per power-on); `UCFrame` and `UCStep`, after `core`'s own hooks, check the signature and `call $D202` / `call $D205` if present. An uninstalled slot 4 is whatever the gap held (`$FF` on fresh SRAM, `$00` after delete-save), neither of which matches the signature. The UC Runtime (README §2a Tier 1b) is our slot-4 code; it owns the other four gaps and loads them itself: `UCWindows` (`core/windows.asm`, 2026-09-21) walks a table of windows on the first runtime step.
